...
class AllocineScraper:
    ...
    def extract_film_details(self, html: str):
        ...
        if not allocine_visa_div:
            # Fallback if text is nested
            allocine_visa_div = next(
                (
                    div for div in soup.find_all('div', class_='item')
                    if div.find('span', class_='what light')
                    and 'N° de Visa' in div.find('span', class_='what light').text
                ),
                None,
            )
        ...
        if raw_duration:
            match = re.search(r'(\d+h\s*\d*\s*min|\d+h|\d+\s*min)', raw_duration.get_text(strip=True))
            duration = match.group(1).replace(' ', '') if match else None
        else:
            duration = None
    ...
    def extract_film_casting(self, html: str) -> dict:
        ...
        for section in other_sections:
            ...
            if section_title not in known_sections:
                print(f"[AllocineScraper] Unknown casting section ignored: {section_title!r}")
                continue
