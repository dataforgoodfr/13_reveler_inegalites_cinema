...
def classify_faces(filtered_detections, batch_size, device):
    ...
    if len(flattened_faces) > 0:
        ages, genders, ethnicities, gender_confs, age_confs, ethnicity_confs = classifier.predict_age_gender_ethnicity(flattened_faces, batch_size = batch_size)
...
def main(source, num_cpu, batch_size, min_area, max_area, min_conf, min_sharpness, max_z, min_mouth_opening, cluster_model, cluster_threshold, agr_method):
    ...
    ### Filter detections
    filtered_detections = filter_detections(detections, effective_area, min_area, max_area, min_conf, min_sharpness, max_z, min_mouth_opening)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--min_conf", type=float, default=0.6, help="Minimum confidence for detection filter")
    parser.add_argument("--min_sharpness", type=float, default=200.0, help="Minimum sharpness for detection filter")
    parser.add_argument("--max_z", type=float, default=-0.2, help="Maximum nose z value for pose filter")
    parser.add_argument("--min_mouth_opening", type=float, default=0.01, help="Minimum upper/lower lips distance for obstruction filter")
    ...
    main(args.source, args.num_cpu, args.batch_size, args.min_area, args.max_area, args.min_conf, args.min_sharpness, args.max_z, args.min_mouth_opening, args.cluster_model, args.cluster_threshold, args.agr_method)