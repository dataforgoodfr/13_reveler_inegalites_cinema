from scripts import VisionDetection, VisionClassifier, Embedding_model, Faces_clustering, DetectionFilter, filter_by_area, filter_by_confidence, filter_by_sharpness, filter_by_pose, get_data_from_file, get_data_from_url, store_predictions_on_video, frame_capture
...
def main() :
    ...
    vision_dectector = VisionDetection(device = device, num_cpu_threads = 8)
    classifier = VisionClassifier(device = device)
    embedding_model = Embedding_model(model = "facenet", device = device)
    
    ### Detect faces in the video
    detections = vision_dectector.crop_areas_of_interest(frames, H_original, W_original, area_type = 'face', batch_size = 256)
    print("Faces detected", datetime.now() - start_time)

    ### Filter detections
    #total_area = H_original * W_original
    effective_height = W_original / 2.4
    effective_area = effective_height * W_original

    filters = DetectionFilter(simple_filters = [filter_by_area, filter_by_confidence], complex_filters = [filter_by_sharpness, filter_by_pose], area_type = 'face', total_area = effective_area, min_area = 0.01, min_sharpness = 200.0, min_conf = 0.6)
    ...
    if len(flattened_faces) > 0:
        genders, ages = classifier.predict_age_gender(flattened_faces, batch_size = 1024)
        ethnicities = classifier.predict_ethnicity(flattened_faces, batch_size = 1024)
    ...
    embedded_faces = embedding_model.get_embedding(batch_size = 1024, detected_faces = flattened_faces)
