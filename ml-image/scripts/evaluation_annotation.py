...
def get_frames(trailer_path: str) -> None:
    ...
    for i in range(len(frames)):
        frame = frames[i]
        text = f'Frame {i}'
        cv2.putText(frame, text, (int(0.8*width), int(0.8*height)),
...
def get_faces(subframes: np.ndarray, visiondetector: vision_detection.VisionDetection) -> list:
    detections = visiondetector.crop_areas_of_interest(
...
def main(trailer_path: str):
    import os
    print(
        f'Outputting numbered trailer video to {trailer_path[:-4]}_with_frames.avi')
