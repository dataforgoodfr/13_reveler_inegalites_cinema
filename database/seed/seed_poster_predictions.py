...
DEFAULT_CSV_PATH = "database/data/machine_learning_predictions/poster_predictions.csv"

def seed_poster_predictions(csv_path: str = DEFAULT_CSV_PATH):
    ...
    try:
        with session.begin():
            ...
            poster_character_repository.bulk_delete_by_film_visas(session, visa_numbers)
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), desc="Seeding poster predictions"):
                ...
                predictions_data = {
                    ...
                    "poster_percentage": float(row["poster_percentage"]) if pd.notna(row["poster_percentage"]) else None
                }
